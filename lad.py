        ...
        for s in range(p+1, self.n+1):
          for l_new in (s, -s):
            ...
            for T__ in T_.get_literals_with_one_term_dropped():
              ...
              if any(T_.eval_at_point(q) for q in self.B_plus):
                if not any(T_.eval_at_point(q) for q in self.B_minus):
                  P.add(T_)
                elif d < D:
                  C[d].append(T_)
            # <>
